...
The nearest neighbour search problem is when provided a set of points and a
search point, return the closest point in the point set to the search point. In
the biased version of this problem a probability distribution over the search
points is available that can be used to answer searches more efficiently on
average. This thesis contains empirical results showing that practical
implementations of data structures for the biased nearest neighbour search
...
neighbour search. The nearest neighbour search problem is to return the
...
avoids problems associated with geometric and numerical robustness when
...
model, where decisions are based upon making comparisons between point
coordinates.

Comparison based data structures are easy to implement and fast in practice for
a moderate number of dimensions, and avoid numerical and geometrical robustness
problems. Empirical evidence suggests that in problems with 16 dimensions or
...
\cite{lsh} where adjacent points are hashed to the same cell, or metric trees,
...
In higher dimensions, the time required to build a Voronoi diagram and the space
required for the point location data structure both depend upon the dimension of
the space in an exponential fashion, which limits both their theoretical and
...
Given a query point $p$, the quadtree node containing $p$, can be found by
...
An improved algorithm for nearest-neighbour search in a kd-tree tree was
described by Friedman et al. \cite{friedman} in 1977 and is described here
rather than the algorithm in the original paper describing kd-trees.
