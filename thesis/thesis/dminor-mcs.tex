...
% TODO: See http://grammar.quickanddirtytips.com/which-versus-that.aspx for quick and dirty tips about using 'which' versus 'that'
...
% TODO: A secondary measure for the odds-on tree will be the cache ``hit rate'', that
...
kdtree began to outperform the kdtree based oddson-tree. In this case, the
variance is also extremely large.
...
the relative performance of the odds-on tree and the kd-tree.
...
\caption{Odds-on Tree Node Counts (2d, pts=10000, sample=10000, build depth=13)}
\end{center}
\end{figure}
...
resulting odds-on tree will be.

A smaller value of build depth results in faster construction times consistently
for both kd-tree and quadtree based odds-on trees. Smaller build depth means
fewer nodes to consider for interferenc queries, which is the primary cost
in constructing the odds-on tree.
%TODO: Typical diagram for construction time

For query times the typical result is that a middle value of build depth is
best. The shallowest trees end up with caches that are too small to be
effective. The larger value of build depth typically does not result in a
significant difference in query time.
%TODO: Typical diagram for query time

Although we typically present results for build time and query time separately,
build depth is a trade off between build time and query time. A typical result
for overall running time is presented below:
%TODO: Typical diagram for total time

%TODO: Difference between sample set size and maximum build depth parameters
...
%TODO:
% - different partitioning strategies for kd-tree
...
% - approximate nn, Approximate Voronoi diagrams

%% applicable to metric trees, locality sensitive hashing?
