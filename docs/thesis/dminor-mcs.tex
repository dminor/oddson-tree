...
search time \(O(n)\). His analysis of Rule II shows that the expected search
time for item in the tree is bounded from above by the entropy \(H\) of the
...
the entropy of the frequency distribution can be constructed in time
...
than biased search trees, and in practice seem do not perform as well as
splaytrees \cite{minor} and so are not discussed further in this thesis. 
...
the query distribution.  Splaytrees, which were invented by Sleator and
Tarjan \cite{splaytree} are a self-modifying binary search tree which perform
well in practice.
...
\subsubsection{Distribution Sensitive Point Location in Convex Subdivions}

TODO

\subsubsection{Biased Range Search}
...
\subsubsection{Instance Optimal Geometric Algorithms}
