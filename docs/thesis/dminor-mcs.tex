...
In some implementations, heuristics are instead used to choose the axis to use
to divide the points, for instance to %TODO
...
% TODO: Emphasize somewhere, early on, that "search time" = "# comparisons"
...
%% TODO: Be more precise here: p_i is the probability that the _answer_ to a query
...
% TODO: In this section, it would be good to give an example of the
...
% TODO: The definition of H needs to be fixed here.  H is a function of the points
...
problems.
% TODO: Again, it's the entropy of the ouput that matters, not the entropy of the 
...
%
% Complicated to implement, results in a compressed quadtree which approximates
% the Voronoi diagram.
% How is that procedure different from approximating the Voronoi diagram more
% directly? Would using a compressed quadtree to approximate the Voronoi
% diagram be any different than what we are doing?
% Yes - because we would be either being approximating the underlying set or
% the query set, -- approximate Voronoi diagram probably gives a bound on the
% error that results, but for our purposes, we need to verify the squares have
% no interference in the ground set anyway, so this approximation factor does
% not help us.
...
experimentation was repeated using each implementation. 

\subsection{Dimension}

The dimension of the point sets is varied across 2, 3, 4, 8 and 16 dimensions.
The upper bound of 16 is mentioned consistently in previous work (TODO refs) as
a practical upper limit beyond which data structures such as the kd-tree do
...
search. The point sets will be generated by sampling a multi-variate uniform
distribution. The number of points in the point set will be varied across
1000, 10000, 100000, and 1000000 points. The multivariate uniform distribution
will be sampled on the range \([-1, 1)\) on each dimension.

%TODO: could try generating the points from a gaussian distribution to see
% and searching with a uniform distribution to see if that has any impact on
% results.

\subsection{Search Set}

The search set is the set of query points which are used to test the odds-on
tree and kd-tree implementations. The search set will be drawn from three
different distributions.

The first is a uniform distribution with the same parameters as the point
set. 

The second is a multi-variate Gaussian distribution. The mean for each
dimension is 0, and sigma is varied from 0.5, 0.1, 0.05, 0.01.

The third is a mixture of multi-variate Gaussian distributions. TODO:
determine parameters for this. I think some sort of grid pattern would be
most interesting.

In each case, the search set will consist of 1000000 points. This number ensures
sufficiently long running times to allow for meaningful comparisons between
conditions.
...
The sample set set size chosen will be $0.1n$, $0.25n$, $0.5n$ and $n$, where
...
This parameter will be set be varied between $0.25\log(n)$, $0.5\log(n)$, and
$\log(n)$ where $n$ is the cardinality of the sample set.
%TODO: before doing a lot of experimentation, do some preliminary work to
%ensure that there isn't an optimal setting for this parameter.

\section{Measuring Performance}

\subsection{Benchmark Platform}

The benchmark platform is an Amazon Web Services EC2 Medium Compute
Optimized instance, with two 64-bit processors and 1.7GB of RAM. Using an EC2
instance increases the repeatability of the experimentation, allows for easy
duplication of hardware to run experiments in parallel, and provides a platform
that is of practical interest. 

\subsection{Performance Metrics}

The primary performance metric is running time. The running time required for
initialization and building the data structure will be measured, as well as
...
% TODO: is Valgrind data actually meaningful in terms of what we are trying to
% accmplish? Might be useful in conclusions to explain unexpected results.
%Since CPU cache utilization can have a major impact on running time, the
%Valgrind \cite{valgrind} cachegrind utility is used to simulate the cache
%performance of the implementation, and data is collected on L1 and L2 cache
%miss rates.

\subsection{Statistical Test}

Each experimental configuration is run ten times, and a Student t-test
is used to test the hypothesis that the odds-on tree provides better
statistically significant better performance than the kd-tree.

The significance criterion for the t-test is $P=0.01$.

For the odds-on tree, the partitioning used to create the sample set will be
determined at random for each experimental run.

%TODO: determine if 10 runs is actually a good number for statistically
% significant results.
