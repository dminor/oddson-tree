...
We choose to use a naive approach of splitting along each axis in sequence.
...
\subsection{Odds-on Tree Construction algorithm}
...
as terminal and the nearest neighbour is stored with the node. The build
process continues until every node in the tree is terminal or all input sample
points have been processed.

TODO: maybe investigate enforcing maximum depth when processing nodes?

\subsection{Odds-on Tree Query algorithm}
