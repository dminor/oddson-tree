...
the distance to furthest point in the result set. The extension to approximate
nearest neighbour searches was described by Arya and Mount \cite{ann}. 
...
Har-Peled \cite{avd} showed TODO 

Arya et al. \cite{arya-avd} extended this to \(t, \epsilon\)-AVD where each
cell in the approximate Voronoi diagram stores \(t\) approximate nearest
neighbours. TODO

In this case, both \(t\) and \(\epsilon\) need to be determined in advance,
unlike approaches using quadtrees or kd-trees where these parameters can
be changd on a query by query basis.
